...
class Mirror extends React.Component<Props, State, UnrealAdapterHook> {
    ...
    async componentDidMount() {     
      ...
      if (this.videoReference.current && this.player) {
        ...
        this.videoReference.current.srcObject = this.player.video.srcObject
    ...
    render() {
        ...
        console.log(`--->      this.options.matchViewPort: ${this.options.matchViewPort}`)
        return (
            <video ref={this.videoReference} id="player" autoPlay style={{"width":'100vw', 'height':'auto'}}>
            </video>
        );
    };
};
