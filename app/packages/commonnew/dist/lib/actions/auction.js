...
const decodeAuction = (buffer) => {
    return (0, borsh_1.deserializeBorsh)(exports.AUCTION_SCHEMA, AuctionData, buffer);
};
...
const decodeBidderPot = (buffer) => {
    return (0, borsh_1.deserializeBorsh)(exports.AUCTION_SCHEMA, BidderPot, buffer);
};
...
const decodeBidderMetadata = (buffer) => {
    return (0, borsh_1.deserializeBorsh)(exports.AUCTION_SCHEMA, BidderMetadata, buffer);
};
...
async function createAuction(winners, resource, endAuctionAt, auctionGap, tokenMint, authority, creator, instructions) {
    const auctionProgramId = (0, ids_1.programIds)().auction;
    const data = Buffer.from((0, borsh_2.serialize)(exports.AUCTION_SCHEMA, new CreateAuctionArgs({
        winners,
        resource,
        endAuctionAt,
        auctionGap,
...
async function startAuction(resource, creator, instructions) {
    const auctionProgramId = (0, ids_1.programIds)().auction;
    const data = Buffer.from((0, borsh_2.serialize)(exports.AUCTION_SCHEMA, new StartAuctionArgs({
        resource,
    })));
    const auctionKey = (await web3_js_1.PublicKey.findProgramAddress([
        Buffer.from(exports.AUCTION_PREFIX),
...
async function placeBid(bidderPubkey, bidderPotTokenPubkey, tokenMintPubkey, transferAuthority, payer, resource, amount, instructions) {
    const auctionProgramId = (0, ids_1.programIds)().auction;
    const data = Buffer.from((0, borsh_2.serialize)(exports.AUCTION_SCHEMA, new PlaceBidArgs({
        resource,
        amount,
    })));
    const auctionKey = (await web3_js_1.PublicKey.findProgramAddress([
