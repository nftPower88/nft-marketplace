...
async function getProgramAuthority() {
    ...
    return (await __1.findProgramAddress([
        Buffer.from(exports.PACKS_PREFIX),
        __1.toPublicKey(PROGRAM_IDS.pack_create).toBuffer(),
    ], __1.toPublicKey(PROGRAM_IDS.pack_create)))[0];
}
...
async function findPackConfigProgramAddress(packSetKey) {
    const PROGRAM_IDS = __1.programIds();
    return (await __1.findProgramAddress([Buffer.from(exports.CONFIG_PREFIX), packSetKey.toBuffer()], __1.toPublicKey(PROGRAM_IDS.pack_create)))[0];
}
...
async function findProgramAddressByPrefix(packSetKey, index, prefix) {
    ...
    return (await __1.findProgramAddress([Buffer.from(prefix), new web3_js_1.PublicKey(packSetKey).toBuffer(), numberBuffer], __1.toPublicKey(PROGRAM_IDS.pack_create)))[0];
}
