...
function MetaProvider({ children = null }) {
    ...
    async function pullItemsPage(userTokenAccounts) {
        ...
        const packsState = shouldEnableNftPacks
            ? await _1.pullPacks(connection, state, wallet === null || wallet === void 0 ? void 0 : wallet.publicKey)
            : state;
    ...
    async function pullPackPage(userTokenAccounts, packSetKey) {
        ...
        const packState = await _1.pullPack({
            connection,
            state,
            packSetKey,
    ...
    async function update(auctionAddress, bidderAddress) {
        ...
        let nextState = await _1.pullPage(connection, page, state, wallet === null || wallet === void 0 ? void 0 : wallet.publicKey, shouldFetchNftPacks);
        console.log('-----> Query started');
