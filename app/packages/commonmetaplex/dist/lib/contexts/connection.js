...
function ConnectionProvider({ children }) {
    ...
    const [savedEndpoint, setEndpoint] = (0, utils_2.useLocalStorageState)('connectionEndpoint', exports.ENDPOINTS[0].url);
    let maybeEndpoint;
    if (networkParam) {
...
function useConnectionConfig() {
    const { endpoint, setEndpoint, env, tokens } = (0, react_1.useContext)(ConnectionContext);
    return {
        setEndpoint,
        endpoint,
        env,
