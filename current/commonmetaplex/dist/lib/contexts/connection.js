...
exports.ENDPOINTS = [
    {
        ...
        label: publicRuntimeConfig.publicSolanaLabel,
        url: publicRuntimeConfig.publicSolanaRpcHost,
...
function ConnectionProvider({ children }) {
    ...
    const [networkStorage, setNetworkStorage] = utils_2.useLocalStorageState('network', DEFAULT_ENDPOINT.name);
    const networkParam = searchParams.get('network');
    const [savedEndpoint, setEndpoint] = utils_2.useLocalStorageState('connectionEndpoint', exports.ENDPOINTS[0].url);
    let maybeEndpoint;
    if (networkParam) {
...
function useConnectionConfig() {
    // const { endpoint, setEndpoint, env, tokens, tokenMap } = useContext(ConnectionContext);
