...
const loadAccountsNoWallet = async (connection, page, tempCache, ownerAddress) => {
    ...
    const queryAuctionCaches = async () => {
        ...
        const auctionCacheData = await getMultipleAccounts_1.getMultipleAccounts(connection, auctionCacheKeys, 'single');
        if (auctionCacheData) {
    ...
    const queryAuctionsFromAuctionManagers = async (parsedAccounts) => {
        ...
        const auctionData = await getMultipleAccounts_1.getMultipleAccounts(connection, [...auctionIds, ...auctionExtendedKeys], 'single');
        if (auctionData) {
    ...
    const queryVaultsForAuctionManagers = async (auctionManagers) => {
        ...
        const vaultData = await getMultipleAccounts_1.getMultipleAccounts(connection, vaultKeys, 'single');
        if (vaultData) {
